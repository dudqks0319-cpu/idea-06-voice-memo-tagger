...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _buildRecommendTab() {
    ...
    return ListView(
      ...
      children: [
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: const [
                Text('음성메모 자동 태그 정리', style: TextStyle(fontWeight: FontWeight.bold)),
                SizedBox(height: 4),
                Text('현재 우선순위 수준에 맞춰 실행 가능한 작업을 우선 추천합니다.'),
                SizedBox(height: 4),
        ...
        EnergySummaryCard(
          currentPriorityLevel: provider.currentPriorityLevel,
          totalMemoSeconds: provider.totalMemoSeconds,
          completedMemoSeconds: provider.completedMemoSeconds,
          onEnergyChanged: provider.setCurrentPriorityLevel,
        ),
        const SizedBox(height: 8),
        if (provider.loading)
          const Card(
          ...
          const Card(
            child: Padding(
              padding: EdgeInsets.all(12),
              child: Text('현재 우선순위 수준에 맞는 추천 작업이 없습니다. 새 작업을 추가해 보세요.'),
            ),
          )
        else
          ...recommended.map(
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        return Scaffold(
          appBar: AppBar(title: const Text('음성메모 자동 태그 정리')),
          body: IndexedStack(
            index: _tabIndex,
            children: [
              _buildRecommendTab(),
