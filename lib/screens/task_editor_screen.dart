...
class _TaskEditorScreenState extends State<TaskEditorScreen> {
  ...
  void initState() {
    ...
    _minutesController = TextEditingController(text: t == null ? '' : t.memoLengthSec.toString());
    _notesController = TextEditingController(text: t?.notes ?? '');
    _priorityLevel = t?.priorityLevel ?? 3;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.existing == null ? '메모 항목 추가' : '메모 항목 수정')),
      body: ListView(
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(labelText: '메모 항목 제목'),
          ),
          const SizedBox(height: 8),
          Text('필요 우선순위: $_priorityLevel / 5'),
          Slider(
            value: _priorityLevel.toDouble(),
            min: 1,
            max: 5,
            divisions: 4,
            onChanged: (v) => setState(() => _priorityLevel = v.round()),
          ),
          const SizedBox(height: 8),
          TextField(
