...
class SupabaseService {
  ...
  Future<UserProfile> fetchProfile(String userId) async {
    if (useSupabase) {
      final data = await Supabase.instance.client
          .from('profiles')
          .select('id, default_priority_level, daily_review_minutes, notifications_enabled')
          .eq('id', userId)
          .maybeSingle();
  ...
  Future<void> updateProfile(UserProfile profile) async {
    if (useSupabase) {
      await Supabase.instance.client.from('profiles').update({
        'default_priority_level': profile.defaultPriorityLevel,
        'daily_review_minutes': profile.dailyReviewMinutes,
  ...
  Future<List<EnergyTask>> fetchTasks(String userId) async {
    if (useSupabase) {
      final rows = await Supabase.instance.client
          .from('voice_memo_tagger_items')
          .select('id, title, priority_level, memo_length_sec, status, notes, created_at, updated_at')
          .eq('user_id', userId)
  ...
  Future<void> saveTask({required String userId, required EnergyTask task}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('voice_memo_tagger_items').upsert(task.toInsertMap(userId), onConflict: 'id');
      return;
    }
  ...
  Future<void> deleteTask({required String userId, required String taskId}) async {
    if (useSupabase) {
      await Supabase.instance.client.from('voice_memo_tagger_items').delete().eq('id', taskId).eq('user_id', userId);
      return;
    }
